...
def main_experiment(pre_trained_model_name, pre_trained_tokenizer_name, dataset_loader, one_minus_lambdas=[1,0.9,0.8,0.7,0.6,0.5,0.4,0.3,0.2,0.1,0], tries=10, demos=[32,16,8,4,2,1,0], repeat = 5):
    ...
    for i in range(0, len(one_minus_lambdas)):
        ...
        table_head[0] = one_minus_lambdas[i]
    ...
    for r in range(0, repeat):
        for i in range(0, len(one_minus_lambdas)):
            ...
            for j in range(0, len(demos)):
                ...
                ACC, MF1, ECE1, output_table = evalute.ICLAcc_evalute(model, tokenizer, dataset_loader, demos[j], tries)
                acc_matrix[i+1][j+1] = ACC
                F1_matrix[i+1][j+1] = MF1
                ECE1_matrix[i+1][j+1] = ECE1

            Tools.message_log.output_single_csv(acc_matrix, extra_name = pre_trained_model_name + ', ' + dataset_loader.dataset_name + ', acc')
            Tools.message_log.output_single_csv(F1_matrix, extra_name = pre_trained_model_name + ', ' + dataset_loader.dataset_name + ', F1')
            Tools.message_log.output_single_csv(ECE1_matrix, extra_name = pre_trained_model_name + ', ' + dataset_loader.dataset_name + ', ECE1')
            del model